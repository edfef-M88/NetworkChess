"""Игровое меню и навигация по режимам."""
...
class GameMenu:
    ...
    def open_singleplayer(self) -> dict:
        """Открыть меню одиночной игры."""
        return {
            "mode": "singleplayer",
            "options": self._get_singleplayer_options()
        }

    def open_multiplayer(self) -> dict:
        """Открыть меню сетевой игры."""
        return {
            "mode": "multiplayer",
            "options": self._get_multiplayer_options()
        }

    def open_local_game(self) -> dict:
        """Открыть локальную игру."""
        return {
            "mode": "local",
            "players": 2,
            "time_control": "10+0"
    ...
    def open_settings(self) -> dict:
        ...
        return {
            "mode": "settings",
            "categories": ["Графика", "Звук", "Управление", "Сеть"]
        }
    ...
    def open_help(self) -> dict:
        ...
        return {
            "mode": "help",
            "topics": ["Правила", "Управление", "Сетевая игра", "О программе"]
        }

    def _get_singleplayer_options(self) -> dict:
        """Получить опции одиночной игры."""
        return {
            "difficulty_levels": [
    ...
    def execute_callback(self, menu_id: str) -> bool:
        """Выполнить callback для пункта меню."""
    ...
    def show_join_room_dialog(self) -> dict:
        """Показать диалог присоединения к комнате."""
        return {
            "available_rooms": [
