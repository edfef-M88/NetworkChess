...
class MatchServer:
    ...
    def _handle_join_room(self, player_id: str, message: dict) -> None:
        """Обработать присоединение к комнате."""
    ...
    def _handle_leave_room(self, player_id: str, message: dict) -> None:
        """Обработать выход из комнаты."""
    ...
    def _handle_resign(self, player_id: str, message: dict) -> None:
        ...
        if room_id in self.rooms:
            self._broadcast_to_room(room_id, {
                "type": "game_end",
                "reason": "resign",
                "winner": self._get_opponent(room_id, player_id)
            })
